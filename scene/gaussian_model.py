...
class MultiGaussianModel:
    """ Work with SAP, where the positions of the splats are not optimized."""
    ...
    def __init__(self, sh_degree : int, configs = None):
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale):
        ...
        fused_point_cloud = pcd[:,0:3] #torch.tensor(np.asarray(pcd[:,0:3])).float().cuda()
        fused_color = RGB2SH(pcd[:,3:6]) #RGB2SH(torch.tensor(np.asarray(pcd[:,3:6])).float().cuda())
        ...
        dist2 = torch.clamp_min(distCUDA2(pcd[:,0:3]), 0.0000001) #*100
        ...
        if self.configs['optimize_GS_pos']:
    ...
    def load_ply(self, path):
        ...
        self._xyz = torch.tensor(xyz, dtype=torch.float, device="cuda") # nn.Parameter(torch.tensor(xyz, dtype=torch.float, device="cuda").requires_grad_(True))
...
class MultiGaussianModelMasked:
    ...
    def __init__(self, sh_degree : int, n_objects: int):
        self.active_sh_degree = 0
        self.max_sh_degree = sh_degree  
        self._xyz = torch.empty(0)
        self._features_dc = torch.empty(0)
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale : float, pcd_mask):
        ...
        self._opacity = nn.Parameter(opacities.requires_grad_(True))
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        #mask for object (each entry is the object ID)
    ...
    def prune_points(self, mask):
        ...
        self._masks = self._masks[valid_points_mask.detach().cpu().numpy()]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_mask):
        ...
        #simply cat the new masks to the existing masks
        self._masks = np.concatenate((self._masks, new_mask), axis = 0)
